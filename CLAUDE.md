...
- Email must render identically regardless of the recipient's dark-mode setting — the eblast template's brand colors are intentional, not something Outlook/Apple Mail/Gmail should be allowed to auto-invert. Two layers, both needed (the meta tags alone were NOT sufficient — confirmed by Tommy still seeing inversion in Outlook after they shipped):
  ...
  2. An explicit `bgcolor="..."` HTML attribute alongside every `background:` inline style on every colored `<table>`/`<td>`/`<body>` in both files — Outlook.com/new Outlook's own auto-dark-mode heuristic specifically treats a bare CSS `background`/`background-color` as "not deliberately chosen" and repaints it, but leaves an element alone if it also carries the legacy `bgcolor` attribute. If a color block is ever added to the template, it needs `bgcolor` added too or it'll be a dark-mode gap. This can't be verified in a browser preview (browsers don't reproduce Outlook's proprietary repaint) — it only shows up by actually receiving the email in Outlook with dark mode on. If `bgcolor` alone still isn't enough for some client, the next escalation is per-element `[data-ogsc]`/`[data-ogsb]` override rules, which need real color values injected per element (bigger change, not done yet).

...
- **The Eblast creator/editor pipeline (drafting, rich-text engine, rendering, push) is frozen — do not touch it for Community-page work.** Tommy explicitly confirmed it's fully working (2026-07) and asked that it be preserved exactly as-is going forward. Community-page changes (Brand panel, Contact panel, field deletions) must stay scoped to `app/communities/[slug]/page.tsx`, `components/ContactPanel.tsx`, `components/BrandPanel.tsx`, `lib/db/*`, and `lib/community-enricher.ts` — never `lib/render-email.ts`, `lib/rich-text/*`, `context/DraftContext.tsx`, or `components/drafter/*`.

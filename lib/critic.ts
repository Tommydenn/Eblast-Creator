...
- Pull-quote fabrication check (BLOCKER): is the pullQuote a sentence the drafter composed rather than a line actually printed on the flyer? Composed marketing copy — value propositions, generic benefit statements ("Experience the warmth of community"), or anything that sounds like ad copy rather than a real flyer line — is fabricated and must be flagged as severity:blocker, category:compliance. Instruction: either replace with a verbatim line from the flyer or leave pullQuote and pullQuoteAttribution empty.
- pullQuoteAttribution fabrication check (BLOCKER): is pullQuoteAttribution populated with a name or title that does not appear in the body paragraphs or flyer? If so, flag as severity:blocker, category:compliance — invented attributions are false.
- Pull-quote echo check: does the pullQuote paraphrase the headline or heroHook? If yes, flag and suggest the most specific/credible sentence from the body as a replacement.
